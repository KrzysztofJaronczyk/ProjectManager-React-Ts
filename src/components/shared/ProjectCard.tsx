  ...
  return (
    <div key={project.id} className="h-48 group relative rounded-md flex flex-col justify-between shadow-slate-900 shadow-md p-4 bg-gradient-to-r from-slate-800 to-slate-700">
      ...
      {isEdit ? (
        ...
        <button className="btn btn-active btn-ghost hidden group-hover:block absolute top-4 right-4 p-0 h-20" onClick={toggleIsEdit}>
          <PencilSquareIcon className="h-6 w-6 top-0 right-0 absolute text-slate-50 cursor-pointer" />
          <TrashIcon onClick={handleDelete} className="h-6 w-6 text-red-900 absolute top-10 right-0 cursor-pointer" />
        </button>
      )}
    </div>
  );
